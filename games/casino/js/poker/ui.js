...
function currentWagerMessage(wagerText, balance) {
...
function exampleCard(rank, suit) {
    return { rank, suit };
}
const HAND_EXAMPLES = {
...
export function createUI(handlers) {
    ...
    const rulesHelp = el('rules-help');
    const rulesDialog = el('rules-dialog');
    const rulesClose = el('rules-close');
    ...
    const outOfChipsDialog = el('out-of-chips-dialog');
    const outOfChipsClose = el('out-of-chips-close');
    // --- paytable (static) ---
    ...
    outOfChipsDialog.addEventListener('click', (event) => {
        if (event.target === outOfChipsDialog)
            outOfChipsDialog.close();
    });
    // Hotkeys: 1-5 hold, Enter deals/draws — how a real machine behaves.
    document.addEventListener('keydown', (event) => {
        if (event.metaKey || event.ctrlKey || event.altKey)
            return;
        if (rulesDialog.open || outOfChipsDialog.open)
            return;
        ...
        if (event.key === 'Enter' || event.code === 'Space') {
            ...
            if (typing || target instanceof HTMLButtonElement || target instanceof HTMLAnchorElement)
                return;
            event.preventDefault();
            handlers.onPrimary();
        }
    ...
    function render(vm) {
        ...
        resultEl.classList.toggle('is-big-win', resolved && HAND_STRENGTH[state.result.rank] >= HAND_STRENGTH[HAND_RANKS.FOUR_OF_A_KIND]);
        if (resolved) {
            const resultText = state.payout > state.wager
                ? `Returned ${chips(state.payout)} chips`
                : state.payout === state.wager
                    ? `Bet returned · break even`
                    : `Lost ${chips(state.wager)} chips`;
            resultEl.replaceChildren(span('result-hand', state.result.label), span('result-amount', resultText));
        }
        else if (dealt) {
    ...
    function showOutOfChips() {
        if (!outOfChipsDialog.open)
            outOfChipsDialog.showModal();
    }
