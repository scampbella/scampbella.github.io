...
export function createUI(handlers: UIHandlers) {
    ...
    const rebuyBtn = el<HTMLButtonElement>('rebuy');

    // --- paytable (static) ---
    const payRows = new Map<HandRank, HTMLTableRowElement>();
    ...
    function render(vm: ViewModel): void {
        ...
        if (resolved) {
            ...
            resultEl.replaceChildren(
                ...
                span(
                    'result-amount',
                    won ? `Win ${chips(state.payout)} chips` : `Lost ${chips(state.wager)} chips`,
                ),
            );
        } else if (dealt) {
