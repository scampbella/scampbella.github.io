...
import type { Card, Hand, Rank } from '../shared/cards.ts';
import { ACE, JACK, cardId } from '../shared/cards.ts';
...
export const PAYTABLE: Readonly<Record<HandRank, number>> = {
...
export const HAND_STRENGTH: Readonly<Record<HandRank, number>> = {
    [HAND_RANKS.NOTHING]: 0,
    [HAND_RANKS.JACKS_OR_BETTER]: 1,
    [HAND_RANKS.TWO_PAIR]: 2,
    [HAND_RANKS.THREE_OF_A_KIND]: 3,
...
export interface HandResult {
    ...
    /** Positions of the cards that EARNED the payout, in displayed order,
     *  ascending. Kickers are excluded. A losing hand is `[]` — including a low
     *  pair, which exists but earns nothing. */
    readonly scoringIndices: readonly number[];
}

const ALL_FIVE: readonly number[] = [0, 1, 2, 3, 4];
 ...
 */
function straightHigh(sorted: readonly number[]): number | null {
    const lo = sorted[0]!;
    const hi = sorted[4]!;
    if (hi - lo === 4) return hi;
    if (lo === 2 && sorted[1] === 3 && sorted[2] === 4 && sorted[3] === 5 && hi === ACE) return 5;
    return null;
}

function result(rank: HandRank, scoringIndices: readonly number[]): HandResult {
    return {
        rank,
 ...
 */
export function evaluate(hand: Hand): HandResult {
    const cards = hand as readonly Card[];
    if (cards.length !== 5) {
    ...
    for (const card of cards) {
        ...
        if (seen.has(id)) throw new Error(`duplicate card in hand: ${id}`);
        seen.add(id);
    }
