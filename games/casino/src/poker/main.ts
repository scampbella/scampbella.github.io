...
import { getBankroll } from '../shared/bankroll.ts';
import { BANKROLL_KEY, POKER_ROUND_KEY } from '../shared/keys.ts';
import { dealDeck } from '../shared/deck.ts';
import { HAND_STRENGTH } from './hands.ts';
...
function currentWager(): number | null {
...
function startRound(): void {
    ...
    if (!bankroll.takeWager(candidate.roundId, wager)) return;
...
const ui = createUI({
    onHold(index) {
        ...
        if (next === state) return; // wrong phase — nothing to redraw
        state = next;
        saveRound();
        render();
    },
    ...
    onPrimary() {
        ...
        if (state.phase === 'betting') startRound();
        else if (state.phase === 'dealt') finishRound();
        else beginNextRound();
    },
